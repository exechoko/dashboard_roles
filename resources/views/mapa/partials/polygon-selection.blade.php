    ...
    `).join('');
        ...
        async function captureMapImage() {
            try {
                ...
                // Guardar estado de clustering y desactivar
                clusterLayers.forEach(layer => {
                    if (layer && layer.disableClustering) {
                ...
                const bounds = currentPolygon.getBounds();
                // Calcular el padding basado en el tamaño del polígono
                const mapSize = mymap.getSize();
                const paddingPercent = 0.15; // 15% de padding
                const paddingPixels = [
                ...
                mymap.fitBounds(bounds, {
                    ...
                    animate: false,
                    maxZoom: 18 // Limitar el zoom máximo para que no se acerque demasiado
                });

                //Expandir clusters
                expandAllClusters();

                // Esperar a que se carguen los tiles y se expandan los clusters
                ...
                if (typeof html2canvas !== 'undefined') {
                    const canvas = await html2canvas(mapElement, {
                        ...
                        ignoreElements: (element) => {
                            // Ignorar elementos ocultos para captura
                ...
                clusterLayers.forEach(layer => {
                    if (layer && layer.enableClustering) {
                        layer.enableClustering();
                    }
                });
