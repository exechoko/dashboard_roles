...
use App\Models\RecursoNovedad;
use App\Models\RecursoNovedadAdjunto;
use App\Models\RecursoNovedadSeguimiento;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class RecursoNovedadController extends Controller
{
    ...
    {
        $novedades = $recurso->novedades()
            ->with(['usuario', 'seguimientos.usuario', 'adjuntos', 'vehiculoReferencia'])
            ->paginate(15);

        $vehiculoActual = $recurso->vehiculoActual();

        return view('flota-911.novedades.index', compact('recurso', 'novedades', 'vehiculoActual'));
    }
    ...
    public function store(Request $request, Recurso $recurso)
    {
        $request->validate([
            ...
            'tipo'          => 'nullable|in:operativa,mecanica',
        ], [
            'descripcion.required'   => 'La descripción es obligatoria.',
            'fecha_novedad.required' => 'La fecha es obligatoria.',
        ]);

        $tipo = $request->input('tipo', 'operativa');
        $vehiculoIdReferencia = null;

        if ($tipo === 'mecanica') {
            $vehiculoIdReferencia = $recurso->vehiculoActual()?->id;
        }

        $novedad = RecursoNovedad::create([
            'recurso_id'            => $recurso->id,
            'tipo'                  => $tipo,
            'vehiculo_id_referencia' => $vehiculoIdReferencia,
            'user_id'               => auth()->id(),
    ...
    public function resolver(RecursoNovedad $novedad)
    {
    ...
    public function storeSeguimiento(Request $request, RecursoNovedad $novedad)
    {
        $request->validate([
            'descripcion' => 'required|string|max:2000',
        ], [
        ...
        RecursoNovedadSeguimiento::create([
            'novedad_id'  => $novedad->id,
            'user_id'     => auth()->id(),
            'descripcion' => $request->descripcion,
        ]);
    ...
    public function storeAdjunto(Request $request, RecursoNovedad $novedad)
    {
        $request->validate([
    ...
    public function destroyAdjunto(RecursoNovedadAdjunto $adjunto)
    {
