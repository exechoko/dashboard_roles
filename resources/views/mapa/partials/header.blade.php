{{-- mapa/partials/header.blade.php --}}
<div id="map-header" class="collapsed">
    <div class="header-content">
        <div class="form-group mb-3">
            {{-- Selector de cámaras --}}
            <div class="form-group">
                <label for="camara_select">Buscar cámara:</label>
                <select name="camara_select" id="camara_select" class="form-control select2 mb-3" style="width: 100%;">
                    <option value="">Seleccione una cámara...</option>
                    @foreach ($camaras as $camara)
                        <option value="{{ $camara['numero'] }}"
                                data-lat="{{ $camara['latitud'] }}"
                                data-lng="{{ $camara['longitud'] }}">
                            {{ $camara['titulo'] }} - {{ $camara['tipo_camara'] }}
                        </option>
                    @endforeach
                </select>
            </div>
