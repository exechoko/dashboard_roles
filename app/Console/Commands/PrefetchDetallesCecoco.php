 ...
 * (cuenta dedicada, ver cecoco.user_prefetch) para no abusar del servidor.
 * Pensado para correr después del import diario.
 */
class PrefetchDetallesCecoco extends Command
{
    ...
    private function procesarRango(CecocoExpedienteService $servicio, Carbon $fechaInicio, Carbon $fechaFin): int
    {
        ...
        $progresoKey = 'cecoco:prefetch-detalles:progreso';
        ...
        $eventos = $this->construirQueryPendientes($fechaInicio, $fechaFin, $refrescar)
            ->orderBy('fecha_hora')
            ->get(['id', 'nro_expediente']);
