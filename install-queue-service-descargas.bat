...
REM Script para instalar el worker de la cola 'descargas' como servicio de Windows
REM (Plataforma de Descargas: mover archivos grandes a su ubicacion final,
REM comprimir ZIPs, generar QRs, enviar notificaciones). Va como servicio
REM aparte de LaravelQueueWorker porque un archivo de varios GB puede tardar
REM minutos en moverse/comprimirse y no debe frenar el resto de la cola
REM 'default' (chat, geocodificacion, etc), igual que LaravelQueueWorkerMbox
REM y LaravelQueueWorkerBackups.
REM Requiere NSSM (Non-Sucking Service Manager)
REM Ejecutar como Administrador

echo ========================================
echo Instalador de Servicio Laravel Queue (descargas)
echo ========================================
echo.
...
REM Configuración
set SERVICE_NAME=LaravelQueueWorkerDescargas
...
REM connection "descargas": sin esto, queue:work usa la conexion default de
REM                    config/queue.php (retry_after=90s) aunque se le diga
REM                    --queue=descargas: el job queda "reservado" con esa
REM                    conexion y a los 90s MySQL lo vuelve a ofrecer como
REM                    disponible aunque siga moviendo/comprimiendo el
REM                    archivo, asi que un segundo intento lo encuentra con
REM                    attempts=1 y lo marca failed sin llegar a correr.
REM                    La conexion 'descargas' (config/queue.php) tiene su
REM                    propio retry_after=7200 para evitar justo esto.
REM --queue=descargas: SOLO procesa la cola dedicada a la Plataforma de Descargas
REM --tries=2        : igual que config/descargas.php (job_tries)
REM --timeout=0      : sin limite por el worker; el Job ya trae su propio
REM                    timeout (7200s, ver config/descargas.php: job_timeout)
...
"%NSSM_PATH%" install "%SERVICE_NAME%" "%PHP_PATH%" "artisan" "queue:work" "descargas" "--queue=descargas" "--sleep=5" "--tries=2" "--timeout=0" "--max-time=3600"
if !errorlevel! neq 0 (
...
"%NSSM_PATH%" set "%SERVICE_NAME%" Description "Laravel Queue Worker dedicado a la cola 'descargas' (Plataforma de Descargas: subida/procesamiento de archivos, ZIPs, QRs, notificaciones)"
...
"%NSSM_PATH%" set "%SERVICE_NAME%" AppStdout "%PROJECT_PATH%\storage\logs\queue-worker-descargas.log"
"%NSSM_PATH%" set "%SERVICE_NAME%" AppStderr "%PROJECT_PATH%\storage\logs\queue-worker-descargas-error.log"
...
echo   - Ver logs:      type "%PROJECT_PATH%\storage\logs\queue-worker-descargas.log"
echo.
