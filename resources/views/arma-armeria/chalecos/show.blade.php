...
@section('content')
    <section class="section">
        <div class="section-header d-flex justify-content-between align-items-center">
            <h3 class="page__heading">Chaleco N° {{ $armeriaChaleco->numero_serie }}</h3>
            <div>
                <a href="{{ route('armas.armeria.chalecos.index') }}" class="btn btn-secondary">
                    <i class="fas fa-arrow-left"></i> Volver
                </a>
                @can('editar-armeria')
                    <a href="{{ route('armas.armeria.chalecos.edit', $armeriaChaleco) }}" class="btn btn-primary">
        ...
        <div class="section-body">
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Información del Chaleco</h4>
                        </div>
                        <div class="card-body">
                            <table class="table table-borderless">
                                <tr>
                                    <th style="width: 20%">Marca / Modelo:</th>
                                    <td style="width: 30%">{{ trim(($armeriaChaleco->marca ?? '') . ' ' . ($armeriaChaleco->modelo ?? '')) ?: '-' }}</td>
                                    <th style="width: 20%">Talle:</th>
                                    <td>{{ $armeriaChaleco->talle ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th>N° de Serie:</th>
                                    <td>{{ $armeriaChaleco->numero_serie }}</td>
                                    <th>Móvil asignado:</th>
                                    <td>{{ $armeriaChaleco->movil ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th>Estado:</th>
                                    <td>
                                        <span class="badge badge-armeria-{{ strtolower(str_replace('_', '-', $armeriaChaleco->estado)) }}">
                                            {{ $armeriaChaleco->estado_label }}
                                        </span>
                                    </td>
                                    <th>Ubicación actual:</th>
                                    <td>
                                        <span class="badge badge-armeria-{{ $armeriaChaleco->ubicacion === 'DIVISION_911' ? 'division' : 'jefatura' }}">
                                            {{ $armeriaChaleco->ubicacion_label }}
                                        </span>
                                    </td>
                                </tr>
                                <tr>
                                    <th>Observaciones:</th>
                                    <td colspan="3">{{ $armeriaChaleco->observaciones ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th>Cargado por:</th>
                                    <td colspan="3">
                                        {{ $armeriaChaleco->creadoPor->name ?? 'Sistema' }} el {{ $armeriaChaleco->created_at->format('d/m/Y H:i') }}
                                    </td>
                                </tr>
                            </table>
                        </div>
                            ...
                            <div class="card-body">
                                <div class="form-row align-items-end mb-3">
                                    <div class="col-auto">
                                        ...
                                        <form action="{{ route('armas.armeria.chalecos.estado', $armeriaChaleco) }}" method="POST" class="form-inline">
                                            @csrf
                                            <select name="estado" class="form-control mr-2">
                                                @foreach (\App\Models\ArmeriaChaleco::ESTADOS as $estado)
                                                    <option value="{{ $estado }}" {{ $armeriaChaleco->estado === $estado ? 'selected' : '' }}>
                                ...
                                @if ($armeriaChaleco->ubicacion === 'DIVISION_911')
                                    <form action="{{ route('armas.armeria.chalecos.enviar-jefatura', $armeriaChaleco) }}" method="POST" class="form-row align-items-end">
                                        @csrf
                                        <div class="col-md-4">
                                ...
                                @else
                                    <form action="{{ route('armas.armeria.chalecos.retornar-division', $armeriaChaleco) }}" method="POST" class="form-row align-items-end">
                                        @csrf
                                        <div class="col-md-4">
                                            <label>Comentario (opcional):</label>
            ...
            <div class="row mt-3">
                <div class="col-md-12">
                    @include('arma-armeria._adjuntos', ['item' => $armeriaChaleco, 'routeBase' => 'armas.armeria.chalecos'])
                </div>
            </div>

            <div class="row mt-3">
                <div class="col-md-12">
                    @include('arma-armeria._movimientos', ['item' => $armeriaChaleco, 'routeBase' => 'armas.armeria.chalecos'])
                </div>
            </div>
        ...
        <div class="modal fade" id="eliminarModal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('armas.armeria.chalecos.destroy', $armeriaChaleco) }}" method="POST">
                        @csrf
                        @method('DELETE')
