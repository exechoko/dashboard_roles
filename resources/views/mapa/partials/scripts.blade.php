...
$(document).ready(function() {
   ...
   // Observar cambios de tema
    ...
    // Ubicar el botón "Vista 3D" a la izquierda del botón "Mapa Satelital"
    posicionarBotonVista3D();
    window.addEventListener('resize', posicionarBotonVista3D);
    if (window.ResizeObserver) {
        var botonSatelital = document.getElementById('toggleMapBtn');
        if (botonSatelital) {
            new ResizeObserver(posicionarBotonVista3D).observe(botonSatelital);
        }
    }
...
function posicionarBotonVista3D() {
    var boton3d = document.getElementById('btn-vista-3d');
    var botonSatelital = document.getElementById('toggleMapBtn');
    if (!boton3d || !botonSatelital) return;

    var rect = botonSatelital.getBoundingClientRect();
    boton3d.style.right = (window.innerWidth - rect.left + 10) + 'px';
    boton3d.style.bottom = (window.innerHeight - rect.bottom) + 'px';
}
...
function loadAntenasMarkers() {
    @foreach ($antenas as $marcador)
        ...
        var popupAntena = escapeHtml(tituloAntena) + (antenaActiva ? '' :
            '<br><strong>Estado:</strong> <span style="color: #dc3545;">INACTIVA</span>');
