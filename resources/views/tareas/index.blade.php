                                    ...
                                    <tbody>
                                        @forelse($items as $item)
                                            <tr>
                                                ...
                                                <td>{{ optional($item->fecha_programada)->format('d/m/Y') }}</td>
                                                <td>
                                                    @php
                                                        $badge = 'secondary';
                                                ...
                                                <td>{{ $item->realizadoPor->name ?? '-' }}</td>
                                                <td>{{ $item->fecha_realizada ? $item->fecha_realizada->format('d/m/Y H:i') : '-' }}</td>
                                                <td>{{ $item->observaciones ? \Illuminate\Support\Str::limit($item->observaciones, 60) : '-' }}</td>
                                                <td style="min-width: 260px;">
                                                    @can('editar-tarea')
                                                        ...
                                                        <div class="btn-group btn-group-sm">
                                                            <a href="{{ route('tareas.edit', $item->tarea_id) }}" class="btn btn-warning" title="Editar tarea">
                                                                <i class="fas fa-edit"></i>
                                                            </a>
                                                            @can('borrar-tarea')
                                                                <form action="{{ route('tareas.destroy', $item->tarea_id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar la tarea y todas sus instancias?');">
                                                                    @csrf
                                                                    @method('DELETE')
